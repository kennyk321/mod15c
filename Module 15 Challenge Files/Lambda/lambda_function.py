...
def get_investment_recs(risk_level): 
    risk_levels= {
        'none': "100% bonds (AGG), 0% equities(SPY)", 
        'low': "60% bonds (AGG), 40% equities(SPY)", 
        'medium': "40% bonds (AGG), 60% equities(SPY)", 
    ...
    if investment_amount is not None: 
        ...
        if investment_amount < 5000:
            return build_validation_result(
                False,
                'investmentAmount',
                'A mininum of $5000 investment is required in order to qualify for this service,'
                'Please re-enter appropriate amount and try again.',
            )
    ...
    if age is not None: 
        ...
        if age <= 0: 
            return build_validation_result(
                False, 
                'age', 
                'Your age is invalid, please try again', 
                'Age range provided should be between 0 and 64',
            )
        elif age >= 65: 
            return build_validation_result(
                False, 
                'age', 
                'Your age exceeds the maximum age and therefore you do not qualify',
                'age range should be below 65'
            )
...
def recommend_portfolio(intent_request):
    ...
    #initial investment rec
    initial_rec= get_investment_recs(risk_level)
    
    #return message with initial rec based on risk level
    return close(intent_request['sessionAttributes'],
    'Fulfilled',
    {
        'contentStyle': 'PlainText',
        'content': 'Based off your risk level -{}. We advise selecting this investment: {}. Thank you, {}, for using our services. '.format(first_name, initial)})
